...
export default function TrashPage() {
    ...
    const handleRestore = async (id: number) => {
        try {
            ...
            setToast({ message: "Đã khôi phục công việc", type: "success" }); // Thêm dòng này
        } catch (error) {
            setToast({ message: "Lỗi khi khôi phục công việc", type: "error" });
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            setToast({ message: "Đã xóa vĩnh viễn công việc", type: "success" }); // Thêm dòng này
        } catch (error) {
            setToast({ message: "Lỗi khi xóa vĩnh viễn", type: "error" });
        }
    };
    ...
    return (
        <div className="p-10 mx-auto">
            ...
            <div className="overflow-x-auto rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div className="flex flex-col items-center justify-between gap-3 border-b border-gray-200 bg-white/90 p-3 shadow-sm md:flex-row">
                ...
                <table className="w-full text-sm text-left text-gray-600 table-fixed min-w-200">
                    <thead className="border-b border-gray-200 bg-gray-100 text-sm text-gray-700">
                        <tr>
                            <th onClick={() => handleSort('title')} className="w-[25%] cursor-pointer px-5 py-4 select-none transition-colors hover:bg-gray-200 group">
                                <div className="flex items-center">Công việc {renderSortIcon('title')}</div>
                            </th>
                            <th onClick={() => handleSort('description')} className="w-[40%] cursor-pointer px-5 py-4 select-none transition-colors hover:bg-gray-200 group">
                                <div className="flex items-center">Nội dung {renderSortIcon('description')}</div>
                            </th>
                            <th onClick={() => handleSort('updatedAt')} className="w-[20%] cursor-pointer px-5 py-4 select-none transition-colors hover:bg-gray-200 group">
                        ...
                        {loading && tasks.length === 0 ? (
                            ...
                            sortedTasks.map(task => (
                                <tr key={task.id} className="border-b border-gray-100 transition-colors hover:bg-gray-100">
                                    <td className="wrap-break-word whitespace-normal px-5 py-4 font-medium text-gray-800">{task.title}</td>
                                    <td className="wrap-break-word whitespace-normal px-5 py-4 text-gray-600">{task.description || '-'}</td>
                                    ...
                                    <td className="whitespace-nowrap px-5 py-4 text-center">
                                        <div className="flex items-center justify-center gap-3">
                                            <button
                                                ...
                                                className="cursor-pointer rounded-md p-1.5 text-green-600 transition-colors hover:bg-green-100 hover:text-green-700"
                                            ...
                                            <button
                                                ...
                                                className="cursor-pointer rounded-md p-1.5 text-red-500 transition-colors hover:bg-red-100 hover:text-red-700"
                                                title="Xóa vĩnh viễn"
                                            >
                                                <Trash2 size={18} />
                                            </button>
            ...
            {/* --- CUSTOM MODAL XÁC NHẬN XÓA VĨNH VIỄN --- */}
                        ...
                        <div className="relative w-full max-w-sm overflow-hidden rounded-3xl border border-red-100 bg-white shadow-[0_25px_80px_-20px_rgba(15,23,42,0.45)]">
                            <div className="bg-linear-to-br from-red-50 via-white to-slate-50 p-6">
                                <div className="flex flex-col items-center text-center mb-8 mt-2">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 mb-4">Xóa vĩnh viễn?</h3>
                                    <p className="text-slate-600 text-sm whitespace-normal wrap-break-word leading-relaxed px-2">
                                        Hành động này <span className="font-semibold text-gray-700">không thể hoàn tác</span>. Bạn có chắc chắn muốn xóa vĩnh viễn công việc này khỏi hệ thống không?
                                    </p>
                                </div>
                                <div className="flex gap-5 w-full">
                                    ...
                                    <button onClick={confirmDelete} className="flex-1 cursor-pointer rounded-xl bg-red-600 py-2.5 text-sm font-semibold text-white shadow-lg shadow-red-600/20 transition-all hover:bg-red-700 hover:shadow-xl">Xóa vĩnh viễn</button>
                                </div>
                            </div>
                        </div>
                    </Transition.Child>
            ...
            <Transition
                show={toast !== null}
                as={React.Fragment}
                // Hiệu ứng xuất hiện: Trượt từ bên phải ra
                enter="transform transition-all ease-out duration-300"
                enterFrom="translate-x-full opacity-0"
                enterTo="translate-x-0 opacity-100"
                // Hiệu ứng biến mất: Trượt ngược về bên phải
