...
export default function TaskList({ refreshTrigger, onRefresh }: TaskListProps) {
    ...
    return (
        <div className="w-full">
            {/* TOOLBAR */}
            <div className="flex flex-col md:flex-row items-center justify-between gap-3 rounded-t-2xl border border-gray-200 border-b-0 bg-white/90 p-3 shadow-sm">
                
                {/* [RESPONSIVE] Thêm flex-col sm:flex-row để các thanh tìm kiếm/lọc tự động xếp dọc trên điện thoại nhỏ */}
                        ...
                        <Listbox value={filterStatus} onChange={setFilterStatus}>
                            {({ open }) => (
                                <div className="relative h-full">
                                    <ListboxButton className="h-full w-full text-left bg-white border border-gray-300 rounded-full px-4 py-2 text-sm text-gray-700 cursor-pointer flex items-center justify-between outline-none hover:border-indigo-400 hover:shadow-sm hover:bg-gray-50 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all group">
                                        <span className="flex items-center gap-2">
                                            <ListFilter size={16} className="text-gray-400 group-hover:text-indigo-500 transition-colors" />
                                            {filterStatus ? STATUS_LIST.find(s => s.value === filterStatus)?.label : 'Tất cả trạng thái'}
                                        </span>
                                        <ChevronDown size={16} className={`text-gray-400 group-hover:text-indigo-500 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
                                    </ListboxButton>
                ...
                <button
                    ...
                    className="flex w-full md:w-auto cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-2xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-600/20 transition-all hover:bg-indigo-700 hover:shadow-xl"
                >
            ...
            {/* BẢNG DỮ LIỆU */}
            <div className="overflow-x-auto rounded-b-2xl border border-gray-200 bg-white shadow-sm">
                {/* [RESPONSIVE] Sửa min-w-200 thành min-w-[900px] để bảng ép chiều rộng chuẩn, kích hoạt vuốt ngang trên điện thoại */}
                <table className="w-full text-sm text-left text-gray-600 table-fixed min-w-225">
                    <thead className="text-sm text-gray-700 bg-gray-100 border-b border-gray-200">
                        <tr>
                            <th onClick={() => handleSort('title')} className="w-[20%] px-5 py-4 cursor-pointer hover:bg-gray-200 select-none group transition-colors">
                                <div className="flex items-center">Công việc {renderSortIcon('title')}</div>
                            </th>
                            <th onClick={() => handleSort('description')} className="w-[36%] px-5 py-4 cursor-pointer hover:bg-gray-200 select-none group transition-colors">
                                <div className="flex items-center">Nội dung {renderSortIcon('description')}</div>
                            </th>
                            <th onClick={() => handleSort('status')} className="w-[20%] px-5 py-4 cursor-pointer hover:bg-gray-200 select-none group transition-colors">
                                <div className="flex items-center">Trạng thái {renderSortIcon('status')}</div>
                            </th>
                            <th onClick={() => handleSort('createdAt')} className="w-[12%] px-5 py-4 cursor-pointer hover:bg-gray-200 select-none group transition-colors">
                    ...
                    <tbody className={loading && tasks.length > 0 ? "opacity-60 transition-opacity pointer-events-none" : "transition-opacity duration-300"}>
                        {loading && tasks.length === 0 ? (
                            ...
                            tasks.map((task) => {
                                ...
                                return (
                                    <tr key={task.id} className={`border-b border-gray-100 transition-colors hover:bg-gray-100 ${rowStateClass}`}>
                                        <td className={`px-5 py-4 font-medium wrap-break-word whitespace-normal ${titleClass}`}>
                                            {task.title}
                                        </td>
                                        <td className={`px-5 py-4 wrap-break-word whitespace-normal ${descriptionClass}`}>
                                            {task.description || '-'}
                                        </td>
                                                                ...
                                                                <ListboxOptions anchor="bottom start" className="[--anchor-gap:4px] w-(--button-width) rounded-xl bg-white py-1 text-sm shadow-2xl ring-1 ring-black/5 focus:outline-none z-50">
                                                                    {STATUS_LIST.map((status) => (
                                                                        <ListboxOption
                                                                            key={status.value}
                                                                            value={status.value}
                                                                            className={({ active }) => `cursor-pointer select-none py-2 px-3 flex items-center gap-2 transition-colors ${active ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-900'}`}
                                                                        >
                                                                            <span className={task.status === status.value ? 'text-indigo-600' : 'text-gray-400'}>{status.icon}</span>
                                                                            <span>{status.label}</span>
                                                                        </ListboxOption>
                                                                    ))}
                                                                </ListboxOptions>
                                        ...
                                        <td className={`px-5 py-4 whitespace-nowrap ${dateClass}`}>
                                            {new Date(task.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                                        </td>
                                        
                                        <td className="px-5 py-4 text-center whitespace-nowrap">
                                            <div className="flex items-center justify-center gap-3">
                                                <button onClick={() => handleEditClick(task)} className="cursor-pointer text-blue-500 hover:text-blue-700 p-1.5 rounded-md hover:bg-blue-100 transition-colors" title="Chỉnh sửa công việc">
                                                    <Pencil size={18} />
                                                </button>
                                                <button onClick={() => handleDeleteClick(task.id)} className="cursor-pointer text-red-500 hover:text-red-700 p-1.5 rounded-md hover:bg-red-100 transition-colors" title="Xóa công việc">
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        </td>
            ...
            <div className="mt-4 flex flex-col gap-3 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex justify-center sm:block">
                    <div className="rounded-full border border-gray-200 bg-white px-4 py-1.5 shadow-sm inline-block">
                        Trang {page} / {totalPages}
                    </div>
                </div>
                <div className="flex justify-center gap-2">
                    <button onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-1.5 transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50">
                        <ChevronLeft size={14} /> Trước
                    </button>
                    <button onClick={() => setPage(p => Math.min(p + 1, totalPages))} disabled={page === totalPages} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-1.5 transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50">
                        Sau <ChevronRight size={14} />
                    </button>
                </div>
            </div>

            {/* POPUP THÊM CÔNG VIỆC MỚI */}
                    ...
                    <Transition.Child as={React.Fragment} enter="transition-all transform ease-out duration-300" enterFrom="opacity-0 scale-95 translate-y-4" enterTo="opacity-100 scale-100 translate-y-0" leave="transition-all transform ease-in duration-200" leaveFrom="opacity-100 scale-100 translate-y-0" leaveTo="opacity-0 scale-95 translate-y-4">
                        <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-[0_25px_80px_-20px_rgba(15,23,42,0.45)]">
                            ...
                            <div className="bg-linear-to-br from-indigo-50 via-white to-slate-50 p-4 sm:p-6">
                                <div className="mb-5">
                                    <h2 className="text-xl font-semibold text-slate-800">Cập nhật thông tin công việc</h2>
                                    <p className="mt-1 text-sm text-slate-500">Điều chỉnh tiêu đề và mô tả cho phù hợp hơn với nhu cầu hiện tại.</p>
                                </div>

                                <form onSubmit={handleEditSubmit} className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="block text-sm font-medium text-slate-700">Tiêu đề</label>
                                        <input type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 text-sm shadow-sm outline-none transition-all focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" placeholder="Tiêu đề" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="block text-sm font-medium text-slate-700">Mô tả</label>
                                        <textarea value={editDescription} onChange={(e) => setEditDescription(e.target.value)} className="min-h-32 w-full rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 text-sm shadow-sm outline-none transition-all focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" placeholder="Mô tả" rows={4} />
                                    </div>
                                    <div className="flex justify-end gap-3 pt-2">
                                        ...
                                        <button type="submit" disabled={isUpdating} className="cursor-pointer rounded-2xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-indigo-600/20 transition-all hover:bg-indigo-700 hover:shadow-xl disabled:cursor-not-allowed disabled:bg-indigo-400">{isUpdating ? 'Lưu...' : 'Lưu thay đổi'}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div className="relative w-full max-w-sm overflow-hidden rounded-3xl border border-red-100 bg-white shadow-[0_25px_80px_-20px_rgba(15,23,42,0.45)]">
                            <div className="bg-linear-to-br from-red-50 via-white to-slate-50 p-6">
                                <div className="flex flex-col items-center text-center mb-8 mt-2">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-100 text-red-600 mb-4 shadow-sm">
                                        <Trash2 size={24} strokeWidth={2.5} />
                                    </div>
                                    <h3 className="text-xl font-bold text-gray-900 mb-4">Xác nhận xóa</h3>
                                    <p className="text-slate-600 text-sm whitespace-normal wrap-break-word leading-relaxed px-2">
                                        Công việc sẽ được giữ trong thùng rác và tự động xóa vĩnh viễn sau <span className="font-semibold text-gray-700">30 ngày</span>. Bạn có chắc chắn muốn tiếp tục?
                                    </p>
                                </div>
                                <div className="flex gap-4 w-full flex-col sm:flex-row">
                                    <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 cursor-pointer rounded-xl bg-gray-200 border-gray-200 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-300">Hủy bỏ</button>
                                    <button onClick={confirmDelete} className="flex-1 cursor-pointer rounded-xl bg-red-600 py-2.5 text-sm font-semibold text-white shadow-lg shadow-red-600/20 transition-all hover:bg-red-700 hover:shadow-xl">Đồng ý xóa</button>
                                </div>
                            </div>
                        </div>
                    </Transition.Child>
            ...
            {/* POPUP THÔNG BÁO (TOAST) GÓC PHẢI TRÊN */}
            <Transition
                show={toast !== null}
                as={React.Fragment}
            ...
            >
                <div className="fixed top-5 right-5 z-50 flex items-center gap-3 bg-gray-900 text-white px-4 py-3 rounded-xl shadow-2xl border border-gray-800">
                    ...
                    <p className="text-sm font-medium pr-2 whitespace-nowrap">{toast?.message}</p>
                    <button onClick={() => setToast(null)} className="text-gray-400 hover:text-white text-xs ml-auto cursor-pointer">✕</button>
                </div>
            </Transition>
        </div>
    );
